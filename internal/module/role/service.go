...
func (s *Service) ListRole(ctx *fiber.Ctx, params ListRoleRequest) ([]ListRoleResponse, int64, error) {
	...
	if err := queryResult.Count(&count).Error; err != nil {
		s.logger.Error("failed to count roles", err)
		return nil, 0, err
	}
	...
	if err := queryResult.
		...
		s.logger.Error("failed to find roles", err)
		return nil, 0, err
	}
...
func (s *Service) CreateRole(ctx *fiber.Ctx, body CreateRoleRequest) error {
	...
	if err := s.db.Gorm.WithContext(ctx.UserContext()).Create(&models.Role{
		Code:        body.Code,
		Name:        body.Name,
		...
		s.logger.Error("failed to create role", err)
		return err
	}
	return nil
}
...
func (s *Service) UpdateRole(ctx *fiber.Ctx, id uint, body CreateRoleRequest) error {
	...
	if err := s.db.Gorm.WithContext(ctx.UserContext()).Model(&models.Role{}).Where("id = ?", id).Updates(&models.Role{
		Code:        body.Code,
		Name:        body.Name,
		Description: body.Description,
		UpdatedAt:   time.Now(),
	}).Error; err != nil {
		s.logger.Error("failed to update role", err)
		return err
	}
	return nil
}
...
func (s *Service) DeleteRole(ctx *fiber.Ctx, id uint) error {
	...
	if err := s.db.Gorm.WithContext(ctx.UserContext()).Delete(&models.Role{}, id).Error; err != nil {
		s.logger.Error("failed to delete role", err)
		return err
	}
	return nil
}
