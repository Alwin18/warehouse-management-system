...
func (h *Handler) ListCarrier(ctx *fiber.Ctx) error {
	var params ListCarrierRequest
	...
	resp, count, err := h.service.ListCarriers(ctx, params)
	if err != nil {
...
func (h *Handler) CreateCarrier(ctx *fiber.Ctx) error {
	var body CreateCarrierRequest
	...
	err := h.service.CreateCarrier(ctx, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) UpdateCarrier(ctx *fiber.Ctx) error {
	...
	err := h.service.UpdateCarrier(ctx, id, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) DeleteCarrier(ctx *fiber.Ctx) error {
	id := formatting.ConvertStringIntoUint(ctx.Params("id"))
	if id == 0 {
		return ctx.Status(fiber.StatusBadRequest).JSON(response.NewErrorResponse(response.ResponseError{
			Message: "Invalid id",
	...
	err := h.service.DeleteCarrier(ctx, id)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
