...
func (h *Handler) ListWarehouse(ctx *fiber.Ctx) error {
	var params ListWarehouseRequest
	...
	resp, count, err := h.service.ListWarehouse(ctx, params)
	if err != nil {
...
func (h *Handler) CreateWarehouse(ctx *fiber.Ctx) error {
	var body CreateWarehouseRequest
	...
	err := h.service.CreateWarehouse(ctx, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) UpdateWarehouse(ctx *fiber.Ctx) error {
	...
	err := h.service.UpdateWarehouse(ctx, id, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) DeleteWarehouse(ctx *fiber.Ctx) error {
	id := formatting.ConvertStringIntoUint(ctx.Params("id"))
	if id == 0 {
		return ctx.Status(fiber.StatusBadRequest).JSON(response.NewErrorResponse(response.ResponseError{
			Message: "Invalid id",
...
func (h *Handler) CreateWarehouseZone(ctx *fiber.Ctx) error {
	var body CreateWarehouseZoneRequest
	if err := ctx.BodyParser(&body); err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(response.NewErrorResponse(response.ResponseError{
			Message: err.Error(),
			Code:    fiber.StatusBadRequest,
	...
	err := h.service.CreateWarehouseZone(ctx, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) CreateWarehouseLocation(ctx *fiber.Ctx) error {
	var body CreateWarehouseLocationRequest
	if err := ctx.BodyParser(&body); err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(response.NewErrorResponse(response.ResponseError{
			Message: err.Error(),
			Code:    fiber.StatusBadRequest,
	...
	err := h.service.CreateWarehouseLocation(ctx, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
