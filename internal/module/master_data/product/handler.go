...
func (h *Handler) ListProduct(ctx *fiber.Ctx) error {
	var params ListProductRequest
	...
	resp, count, err := h.service.ListProduct(ctx, params)
	if err != nil {
...
func (h *Handler) CreateProduct(ctx *fiber.Ctx) error {
	...
	err := h.service.CreateProduct(ctx, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) UpdateProduct(ctx *fiber.Ctx) error {
	...
	err := h.service.UpdateProduct(ctx, id, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
