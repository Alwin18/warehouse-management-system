...
func (h *Handler) ListSupplier(ctx *fiber.Ctx) error {
	var params ListSupplierRequest
	...
	resp, count, err := h.service.ListSupplier(ctx, params)
	if err != nil {
...
func (h *Handler) CreateSupplier(ctx *fiber.Ctx) error {
	...
	err := h.service.CreateSupplier(ctx, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) UpdateSupplier(ctx *fiber.Ctx) error {
	...
	err := h.service.UpdateSupplier(ctx, id, body)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
...
func (h *Handler) DeleteSupplier(ctx *fiber.Ctx) error {
	id := formatting.ConvertStringIntoUint(ctx.Params("id"))
	if id == 0 {
		return ctx.Status(fiber.StatusBadRequest).JSON(response.NewErrorResponse(response.ResponseError{
			Message: "Invalid id",
	...
	err := h.service.DeleteSupplier(ctx, id)
	if err != nil {
		return errors.HandleError(ctx, err)
	}
